import os, glob, numpy as np, pandas as pd, scanpy as sc, scipy.sparse as sp_sparse, anndata
...
for design in all_designs:
	...
	# Find sgRNAs in MOI = 1 cells
	assert(np.all(np.equal(np.nonzero(curr_design_mat[MOI1_inds, :])[0], np.arange(MOI1_inds.size)))) # One nonzero sgRNA per cell
	sgRNA_inds = np.nonzero(curr_design_mat[MOI1_inds, :])[1]
	assert(sgRNA_inds.size == MOI1_inds.size)

	curr_sgRNAs = np.full(curr_RNA_barcs.size, '', dtype=object)
	curr_sgRNAs[MOI1_inds] = sgRNA_names[sgRNA_inds]
