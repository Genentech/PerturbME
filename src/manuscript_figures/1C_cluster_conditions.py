...
fig.savefig(os.path.join(fig_pn, '1C_UMAP_RNA_CITE_high_condition.pdf'), bbox_inches='tight', dpi=800)
plt.close()
fig, ax = plt.subplots()
sc.pl.umap(adata_RNA_CITE[low_inds, :], color=['Condition'], ax=ax)
fig.savefig(os.path.join(fig_pn, '1C_UMAP_RNA_CITE_low_condition.pdf'), bbox_inches='tight', dpi=800)
plt.close()
fig, ax = plt.subplots()
sc.pl.umap(adata_RNA_CITE[control_inds, :], color=['Condition'], ax=ax)
fig.savefig(os.path.join(fig_pn, '1C_UMAP_RNA_CITE_control_condition.pdf'), bbox_inches='tight', dpi=800)
plt.close()
...
fig.savefig(os.path.join(fig_pn, '1D_clustering/UMAP_RNA_CITE_HLA_A.pdf'), bbox_inches='tight', dpi=800)
plt.close()
