...
def main():
    ...
    fig_pn = os.path.join(full_exp_pn, 'figures/CROP/linear_model/%s_cells_per_target/all_features' % cells_per_target_thresh)
    ...
    #     fig.savefig(os.path.join(fig_pn, 'find_k_covs.png'), bbox_inches='tight', dpi=800)
    #     plt.close()
    #     fig, ax = plt.subplots()
    #     ax.plot(K_features, inertia_features, marker='.')
    #     ax.set_xlabel('# Clusters'), ax.set_ylabel('Inertia'), ax.set_title('Features')
    #     ax.set_xticks(np.arange(0, 22, 2))
    #     fig.savefig(os.path.join(fig_pn, 'find_k_features.png'), bbox_inches='tight', dpi=800)
    #     plt.close()
    #
    #
    #
	...
	# Delete covaraite cluster 0 & feature clusters 0 + 7
    if False:
        covs_clusters_to_delete = np.array([1])
        features_clusters_to_delete = np.array([0, 7])

        cov_inds_to_delete = np.where(np.isin(covs_cluster_labels, covs_clusters_to_delete))[0]
        feature_inds_to_delete = np.where(np.isin(features_cluster_labels, features_clusters_to_delete))[0]

        covs_order = np.delete(covs_order, cov_inds_to_delete)
        features_order = np.delete(features_order, feature_inds_to_delete)

        covs_cluster_labels = np.delete(covs_cluster_labels, cov_inds_to_delete)
        features_cluster_labels = np.delete(features_cluster_labels, feature_inds_to_delete)

    if plot:
        plot_clustermap(filtered_B, features_order, covs_order, filtered_features, filtered_covs,
                        os.path.join(fig_pn, 'beta_%d_features_%d_covs.png' % (num_clusters_features, num_clusters_covs)),
        ...
        plot_clustermap(B_correlation_covs, covs_order, covs_order, filtered_covs, filtered_covs,
                        os.path.join(fig_pn, 'beta_corr_covs_%d_clusters.png' % (num_clusters_covs)),
        ...
        plot_clustermap(B_correlation_features, features_order, features_order, filtered_features, filtered_features,
                        os.path.join(fig_pn, 'beta_corr_features_%d_clusters.png' % (num_clusters_features)),
        ...
        fig.savefig(os.path.join(fig_pn, 'beta_corr_covs_%d_clusters_high_label_arr.png' % (num_clusters_covs)), bbox_inches='tight', dpi=800)
        plt.close()
        fig, ax = plt.subplots()
        ax_ret = ax.imshow(np.reshape(cells_low_label_arr, (-1, 1)), aspect=(1/100), cmap='Blues', clim=[0, 30])
        ax.set_xticks([]), ax.set_yticks([])
        fig.savefig(os.path.join(fig_pn, 'beta_corr_covs_%d_clusters_low_label_arr.png' % (num_clusters_covs)), bbox_inches='tight', dpi=800)
        plt.close()


    bp()
    pd.DataFrame({'Cluster' : features_cluster_labels, 'Feature' : filtered_features[features_order]}).to_csv(os.path.join(fig_pn, 'beta_corr_features_%d_clusters_list.csv' % (num_clusters_features)), index=False)
    pd.DataFrame({'Cluster' : covs_cluster_labels, 'Covariate' : filtered_covs[covs_order]}).to_csv(os.path.join(fig_pn, 'beta_corr_covs_%d_clusters_list.csv' % (num_clusters_covs)), index=False)
    pd.DataFrame(data=filtered_B, index=filtered_features, columns=filtered_covs).to_csv(os.path.join(fig_pn, 'beta_%d_features_%d_covs.csv' % (num_clusters_features, num_clusters_covs)))

    #
    # save_pn = os.path.join(load_pn, '%s_cells_per_target/all_features/GO_analysis/' % cells_per_target_thresh)
    #
    # if True:
    ...
    # ax.set_xticks([]), ax.set_yticks([])
    # fig.savefig(os.path.join(fig_pn, 'HLA_label_%d_features.png' % (num_clusters_features)), bbox_inches='tight', dpi=800)
    # plt.close()
    ...
    for num_clusters_covs in cov_clusters_to_test:
        ...
        if plot:
            ...
            ax.set_xticks([]), ax.set_yticks([])
            fig.savefig(os.path.join(fig_pn, 'beta_corr_covs_%d_clusters_label.png' % (num_clusters_covs)), bbox_inches='tight', dpi=800)
            plt.close()
    ...
    #     fig.savefig(os.path.join(fig_pn, 'covs_%d_clusters_high_label_arr_cmap.png' % (num_clusters_covs)), bbox_inches='tight', dpi=800)
    #     plt.close()
    #
    #     fig, ax = plt.subplots(figsize=(1, 12))
    #     fig.subplots_adjust(bottom=0.5)
    ...
    #     fig.savefig(os.path.join(fig_pn, 'covs_%d_clusters_low_label_arr_cmap.png' % (num_clusters_covs)), bbox_inches='tight', dpi=800)
    #     plt.close()
    ...
    for num_clusters_features in feature_clusters_to_test:
        ...
        if plot:
            ...
            ax.set_xticks([]), ax.set_yticks([])
            fig.savefig(os.path.join(fig_pn, 'beta_corr_features_%d_clusters_label.png' % (num_clusters_features)), bbox_inches='tight', dpi=800)
            plt.close()
