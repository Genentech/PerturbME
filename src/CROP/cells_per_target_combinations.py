...
all_design_mats = glob.glob(os.path.join(design_dir, '*.target_design_mat.%d_%d_%.2f_%d_%d_%d.npz' % (READS_THRESH,READS_PERC_THRESH,PERC_THRESH,UMI_COUNT_THRESH,UMI_READS_THRESH,FILTERED_UMI_THRESH)))
...
for curr_mat in all_design_mats:
	...
	if 'CTL' not in cond:
		...
		design_mat = np.array(sp_sparse.load_npz(curr_mat).todense())

		assert(design_mat.shape[1]==target_names.size)

		MOI = design_mat.sum(1)
		MOI2_cells = np.where(MOI == 2)[0]

		for MOI2_cell in MOI2_cells:
			doublet_target_inds = np.where(design_mat[MOI2_cell, :].flatten() != 0)[0]
			assert(doublet_target_inds.size == 2)

			curr_target_names = np.sort(target_names[doublet_target_inds]) # Sort so dictionary key is consistent
			assert(curr_target_names.size == 2)

			dict_key = curr_target_names[0]+'-'+curr_target_names[1]
...
cells_per_combo_low_names = np.fromiter(HLA_low_dict.keys(), dtype='<U100')
cells_per_combo_high_names = np.fromiter(HLA_high_dict.keys(), dtype='<U100')
